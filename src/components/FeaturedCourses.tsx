    ...
    return (
        <div className='py-12 bg-gray-900'>
                    ...
                    {featuredCourses.map((course: Course) => (
                        <div key={course.id} className="flex justify-center">
                            <BackgroundGradient className="flex flex-col rounded-[22px] max-w-sm p-4 sm:p-10 bg-white dark:bg-zinc-900 overflow-hidden h-full ">
                                <img
                                    className="w-full h-48 object-cover rounded-[22px] mb-4"
                                    src={course.image}
                                    alt={course.title}
                                />
                                ...
                                <Link href={`/courses/${course.slug}`} className="mt-4 text-teal-600 hover:text-teal-700 font-semibold text-center">Learn More</Link>
                            </BackgroundGradient>
                        </div>
                    ))}
                </div>
            ...
            <div className='mt-20 text-center'>
                <Link href='/courses' className='px-4 py-2 rounded border border-neutral-600 text-neutral-800 bg-white hover:bg-gray-100 transition duration-200'>View All Courses</Link>
            </div>
        </div>
    )
}
